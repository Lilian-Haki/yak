...
if (empty($_SESSION['YAK'])) {
    ...
    if (isset($_GET['qid'])) {
        $id=$_GET['qid']/99970;
        if(mysqli_query($conn, "UPDATE mentor set status=1 where id='".$id."'")){
        echo "<script>alert('Account Approved.')</script>";
        echo "<script>setTimeout(\"location.href = 'menn';\",50);</script>";
        }else{
            echo "<script>alert('Failed.')</script>"; 
        }
    }
    if (isset($_POST['btnRe'])) {
        if(mysqli_query($conn, "UPDATE mentor set status=2,remarks='".$_POST['remarks']."' where id='".$_POST['id']."'")){
            ...
            <div class="panel-body">
                <div class="table-sorting table-responsive">
                    <table class="table table-striped table-bordered table-hover" id="tSortable22">
                        ...
                        <tbody>
                            <?php
                  $q = $conn->query("select * from mentor where (status=0 or status=3)");
                  $i=1;
                  while($r = $q->fetch_assoc())
                  {     ?> 
                  <tr>
                    <td><?php echo $i;?></td>
                    <td><?php echo $r['mentor_id'];?></td>
                    ...
                    <td>
                        <?php if($r['status']==0){
                            ?>
                      <a href="menn?qid=<?php echo ($r['id']*99970); ?>"
