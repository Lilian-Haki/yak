...
if (empty($_SESSION['YAK'])) {
    ...
    if (isset($_GET['qid'])) {
        $id=$_GET['qid']/99970;
        if(mysqli_query($conn, "UPDATE partner set status=1 where id='".$id."'")){
        echo "<script>alert('Account Approved.')</script>";
        echo "<script>setTimeout(\"location.href = 'parn';\",50);</script>";
        }else{
            echo "<script>alert('Failed.')</script>"; 
        }
    }
    if (isset($_POST['btnRe'])) {
        if(mysqli_query($conn, "UPDATE partner set status=2,remarks='".$_POST['remarks']."' where id='".$_POST['id']."'")){
            ...
            <div class="panel-body">
                <div class="table-sorting table-responsive">
                    <table class="table table-striped table-bordered table-hover" id="tSortable22">
                        ...
                        <tbody>
                            <?php
                  $q = $conn->query("select * from partner where (status=0 or status=3)");
                  $i=1;
                  ...
                  <tr>
                    ...
                    <td>
                        <?php if($r['status']==0){
                            ?>
                      <a href="parn?qid=<?php echo ($r['id']*99970); ?>"
