...
if (empty($_SESSION['YAK'])) {
    ...
    if (isset($_GET['qid'])) {
        $id=$_GET['qid']/99970;
        if(mysqli_query($conn, "UPDATE staff set status=1 where id='".$id."'")){
        echo "<script>alert('Account Approved.')</script>";
        echo "<script>setTimeout(\"location.href = 'sern';\",50);</script>";
        }else{
            ...
            <div class="panel-body">
                <div class="table-sorting table-responsive">
                    <table class="table table-striped table-bordered table-hover" id="tSortable22">
                        ...
                        <tbody>
                            <?php
                  $q = $conn->query("select * from staff where role='Service Manager' and (status=0 or status=3)");
                  $i=1;
                  ...
                  <tr>
                    ...
                    <td>
                        <?php if($r['status']==0){
                            ?>
                      <a href="sern?qid=<?php echo ($r['id']*99970); ?>"
